...
import warnings
import traceback
from swutil import files
from swutil.collections import unique
from swutil.decorators import add_runtime
...
def conduct(func, experiments=None, name=None, path='scilog', supp_data=None,
    ...
    MSG_GIT_DONE = 'Created git commit {} in branch {} as snapshot of current state of git repository. Check {}'.format('{}',STR_GIT_SCILOG,'{}')
...
def _run_single_experiment(arg):
    ...
    try:
        ...
        if memory_profile:
            _store_text(memory_profile_files(i), m.getvalue())
...
def load(search_pattern='*', path='', ID=None,no_results=False, need_unique=True):
    ...
    info.pkl and results.pkl as well as the directory of the labook entry
